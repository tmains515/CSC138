...
def handleClient(client_address, client_socket):
    ...
    try:
        ...
        while True:
            ...
            # Separates the command and message components
            ...
            match command:
                case "JOIN":
                    ...
                    if username in active_users:
                        ...
                        client_socket.send(f"You are now registered \n".encode())
                ...
                case "BCST":
                    ...
                    if username in active_users:
                        ...
                        client_socket.send(f"ERROR: Must be logged in to do this".encode())
                # Default case, command not known
                case _:
                    client_socket.send(f"ERROR: Command -{command}-  not known".encode())

    except Exception as e:
        print(f"Client error: {e}")


    # For sudden disconnect or process killed without crashing server
    finally:

        # username shows active client
        if username:
